...
class ScientificV3Writer:
    ...
    def append_runtime_tick(self, runtime: Any) -> int:
        ...
        with self._lock:
            ...
            with _prof_span("sci_v3_capture"):
            ...
            def _enq(key: str, rows: list[dict[str, Any]]) -> None:
                ...
                for row in rows:
                    ...
                    nbytes += len(line) + 1
                    nrec += 1
                    self._bufs[key].append(line)
                    self._counts[key] += 1
                _prof_count(f"sci_v3_records_{key}", nrec)
                _prof_count(f"sci_v3_bytes_{key}", nbytes)

            with _prof_span("sci_v3_serialize"):
            ...
            if self._queue_depth() >= self.flush_every:
                with _prof_span("sci_v3_write"):
                    with _prof_span("sci_v3_flush"):
                        self._flush_unlocked()
