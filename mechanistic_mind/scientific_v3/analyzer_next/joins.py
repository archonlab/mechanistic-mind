...
def apply_joins(
    ...
    *,
    ...
    for s in stories:
        ...
        surf_keys = [c for c in s.observation_components if str(c.get("key", "")).startswith("surface_c")]
        if surf_keys:
            vals = [float(c["value"]) for c in surf_keys if isinstance(c.get("value"), (int, float))]
            contrast = (max(vals) - min(vals)) if vals else 0.0
            ctx.append({
                "kind": "SURFACE_EXPOSURE",
                "layer": "OBSERVED",
                "channels": [c["key"] for c in surf_keys],
                "optical_contrast": contrast,
                "note": "FOV-gated anonymous optical channels; not terrain labels",
            })
            if contrast > 1e-6:
                ctx.append({
                    "kind": "OPTICAL_CONTRAST_CHANGE",
                    "layer": "DERIVED",
                    "optical_contrast": contrast,
                })
            if s.motor_id:
                ctx.append({
                    "kind": "ACTION_UNDER_SURFACE_EXPOSURE",
                    "layer": "DERIVED",
                    "channels": [c["key"] for c in surf_keys],
                    "note": "Motor occurred while surface_c* were in accessible observation; causation NOT_ESTABLISHED",
                })
