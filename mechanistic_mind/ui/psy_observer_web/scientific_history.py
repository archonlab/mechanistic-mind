...
def _compact_vision_optical_for_slot(
    ...
    *,
    ...
    try:
        from mechanistic_mind.research.tick_profiler import span as _prof_span
        with _prof_span("sci_vision_optical"):
        ...
        return {
            ...
            "visual_surface_discrimination": sample.get("visual_surface_discrimination") or "OFF",
    ...
    def append_tick(self, runtime: Any) -> int:
        ...
        with self._lock:
            ...
            if tick <= self._last_tick_written:
                return 0
            nbytes = 0
            with _prof_span("sci_serialize"):
                if self.telemetry_mode == TELEMETRY_MODE_V2:
                    for row in rows:
                        line = _jsonl_line(compact_tick_row_v2(row))
                        nbytes += len(line)
                        self._tl_buf.append(line)
                    if self.checkpoint_every > 0 and tick % self.checkpoint_every == 0:
                        cp = build_checkpoint_v2(tick=tick, full_rows=rows)
                        line = _jsonl_line(cp)
                        nbytes += len(line)
                        self._cp_buf.append(line)
                        self._checkpoints_written += 1
                else:
                    for row in rows:
                        line = _jsonl_line(row)
                        nbytes += len(line)
                        self._tl_buf.append(line)
            _prof_count("sci_bytes", nbytes)
            self._last_tick_written = tick
            self._rows_written += len(rows)
            if len(self._tl_buf) >= self.flush_every:
                with _prof_span("sci_write"):
                    with _prof_span("sci_v2_flush"):
    ...
    def _flush_unlocked(self) -> None:
        if self._tl_buf:
            with self.timeline_path.open("a", encoding="utf-8") as fh:
                for line in self._tl_buf:
                    fh.write(line)
                    fh.write("\n")
            self._tl_buf.clear()
        if self._ev_buf:
            with self.events_path.open("a", encoding="utf-8") as fh:
                for line in self._ev_buf:
                    fh.write(line)
                    fh.write("\n")
            self._ev_buf.clear()
        if self._cp_buf:
            with self.checkpoint_path.open("a", encoding="utf-8") as fh:
                for line in self._cp_buf:
                    fh.write(line)
                    fh.write("\n")
            self._cp_buf.clear()
        self._write_meta_unlocked(flush_bufs=False)
