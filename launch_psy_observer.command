#!/usr/bin/env bash
# Psy Observer Web — Beta 3 launcher (macOS Finder double-clickable).
...
if [ ! -f "$ROOT/mechanistic_mind/ui/psy_observer_web/web_dist/index.html" ]; then
  fail "Missing production web_dist. Re-download the Beta 3 archive."
fi
if [ ! -f "$BOOTSTRAP" ]; then
...
cd "$ROOT"
echo "Starting Psy Observer Web (Beta 3)…"
