...
Later forensic work found experiment Apply/Reset could run under a different
mechanism set than the researcher’s other Experiment tabs. Measurements here
stay valid for the configuration that was on the process. Do not treat them as
the canonical Beta 3.1 baseline unless that process matched `TIKTAALIK_BETA31`.
Profile artifacts were not deleted.
