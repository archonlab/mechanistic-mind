...
export function VisionBars({
  ...
  const hasKeys =
    ...
    && ('exo_0' in observation || 'exo_1' in observation || 'exo_2' in observation
      || 'spatial_exo_a0' in observation);
  const authorityOn =
    visionEnabled === true
    || (visionEnabled == null && hasKeys);
  const mode = String(spatialVision || 'LEGACY').toUpperCase();
  ...
  const spat = [0, 1, 2, 3, 4].map((i) => Number(observation?.[`spatial_exo_a${i}`] ?? 0));
  const max = Math.max(exo0, exo1, exo2, ...spat, 1e-9);
  const bar = (v: number) => {
  ...
  if (spatial) {
    return (
      <div className="vision-bars" title="Agent-accessible spatial_exo_a* — not World GT">
        <div className="vision-label">
          {prefix}VISION ON · {mode} A0–A4 <span className="info-kind kind-AGENT_ACCESSIBLE">AGENT</span>
        </div>
        {spat.map((v, i) => (
          <div className="vision-row" key={i}>
            <span>A{i}</span>
            <code>{bar(v)}</code>
            <span className="vnum">{v.toFixed(3)}</span>
          </div>
        ))}
        <div className="subtle">
          spatial_exo_a0…a4 · 3-sector exo_0={exo0.toFixed(3)} exo_1={exo1.toFixed(3)} exo_2={exo2.toFixed(3)}
        </div>
      </div>
    );
  }
