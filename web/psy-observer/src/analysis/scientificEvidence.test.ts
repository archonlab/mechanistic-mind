...
describe('scientific evidence Analyzer', () => {
  ...
  it('Analyze Current job summary reconstructs ticks without scientific_rows', () => {
    const pkg = {
      ...
      beta31_vision_report_text: 'BETA 3.1 VISION ANALYSIS\n========================\nCONFIGURATION\n',
    ...
    assert.equal(Number(a0?.vision?.foreign_body_exposure_ticks), 8);
    assert.ok(a.analysis_log.includes('LEGACY_BODY_VISUAL_EXPOSURE'));
    assert.ok(a.analysis_log.includes('BETA 3.1 VISION ANALYSIS'));
  });
