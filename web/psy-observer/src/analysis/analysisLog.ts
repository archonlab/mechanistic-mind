...
export function formatAnalysisLog(analysis: RunAnalysis): string {
  ...
  for (const a of analysis.agents) {
    ...
    if (a.vision) {
      ...
      lines.push('    (LEGACY_BODY_VISUAL_EXPOSURE if reconstructed; not SURFACE_OPTICAL_EXPOSURE / SPATIAL_OPTICAL_STRUCTURE)');
  ...
  lines.push(...vfLines);
  const histAgents: any = (analysis as any).canonical_history_agents || {};
  const reconNotes: string[] = [];
  for (const a of analysis.agents) {
    const row = histAgents[a.agent_id];
    const n = row?.visual_exposure_ticks ?? row?.legacy_body_visual_exposure_ticks;
    if (n != null && Number(n) > 0) {
      reconNotes.push(`${a.agent_id}: LEGACY_BODY_VISUAL_EXPOSURE ticks=${n} (V3+geometry reconstruction; ≠ SURFACE/SPATIAL occupancy)`);
    }
  }
  if (reconNotes.length) {
    lines.push('  Reconstruction (authoritative when live optical series is NOT AVAILABLE):');
    for (const n of reconNotes) lines.push(`    ${n}`);
    lines.push('  cognition linkage remains NOT_ESTABLISHED (no recognition claim).');
  }
  const b31 = (analysis as any).beta31_vision_report_text;
