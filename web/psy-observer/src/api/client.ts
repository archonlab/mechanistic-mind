...
export async function getCheckpointStatus() {
  const r = await fetch(`${API}/api/checkpoint/status`);
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}

export async function setCheckpointCadence(every_ticks: number) {
  const r = await fetch(`${API}/api/checkpoint/cadence`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ every_ticks }),
  });
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}
...
export async function checkpointNow() {
  const r = await fetch(`${API}/api/checkpoint/now`, { method: 'POST' });
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}

export async function restoreCheckpoint() {
  const r = await fetch(`${API}/api/checkpoint/restore`, { method: 'POST' });
  if (!r.ok) throw new Error(await r.text());
  return r.json();
}
